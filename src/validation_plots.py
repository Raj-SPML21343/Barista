...
def sgd_comparison(dataset_name):
    print("Running SGD Comparison for {}".format(dataset_name))
...
def variation_of_m(dataset_name):
    print("Running SVRG Comparison with varying `m` for {}".format(dataset_name))
    ...
    _, fvals_bb1, stepsizes_bb1 = svrg_bb(grad, 1, n, d, m = int(0.5 * n), func=func, max_epoch=MAX_EPOCH, retFvals=True)
    _, fvals_bb2, stepsizes_bb2 = svrg_bb(grad, 1, n, d, m = n, func=func, max_epoch=MAX_EPOCH, retFvals=True)
    _, fvals_bb3, stepsizes_bb3 = svrg_bb(grad, 1, n, d, m = 2*n, func=func, max_epoch=MAX_EPOCH, retFvals=True)
    _, fvals_bb4, stepsizes_bb4 = svrg_bb(grad, 1, n, d, m = 4*n, func=func, max_epoch=MAX_EPOCH, retFvals=True)

    plt.figure(figsize=(8, 4))
    plt.subplot(1, 2, 1)
    plt.plot(fvals_cst1 - fvals_bb1[-1], '--', label = r'$m = 0.5n$')
    plt.plot(fvals_cst2 - fvals_bb1[-1],  '--', label = r'$m = n$')
    plt.plot(fvals_cst3 - fvals_bb1[-1], '--', label = r'$m = 2n$')
    plt.plot(fvals_cst4 - fvals_bb1[-1], '--', label = r'$m = 4n$')
    plt.yscale("log")
    plt.xlim([0, 25])
    plt.ylim([10**(-14), 10**0])
    plt.title("Sub-optimality of SVRG for " + dataset_name)
    ...
    plt.savefig("./results/m_comparison_{}.png".format(dataset_name))
    plt.show()
...
if __name__ == "__main__":
    ...
    # sgd_comparison("w8a")
    # sgd_comparison("ijcnn1")
    # sgd_comparison("rcv1.binary")

    # variation_of_m("w8a")
    # variation_of_m("ijcnn1")
    # variation_of_m("rcv1.binary")