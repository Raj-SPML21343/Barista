...
def variation_of_m(dataset_name):
    ...
    plt.figure(figsize=(8, 4))
    plt.subplot(1, 2, 1)
    plt.plot(fvals_bb1 - fvals_bb1[-1], '--', label = r'$m = 0.5n$')
    plt.plot(fvals_bb2 - fvals_bb1[-1],  '--', label = r'$m = n$')
    plt.plot(fvals_bb3 - fvals_bb1[-1], '--', label = r'$m = 2n$')
    plt.plot(fvals_bb4 - fvals_bb1[-1], '--', label = r'$m = 4n$')
    plt.yscale("log")
    plt.xlim([0, 25])
    plt.ylim([10**(-14), 10**0])
    plt.title("Sub-optimality of SVRG for " + dataset_name)
